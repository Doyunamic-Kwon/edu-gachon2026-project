...
작성일: 2026-07-09 (최종 갱신: 2026-07-13, 세션 히스토리·추천 질문 실제 계약 반영+배포 후) · 가천대 2026 AI 부트캠프 team07 · 담당: 이후윤 (서버/DB/프론트엔드/백엔드/CI-CD)
...
| 영역 | 기술 |
|---|---|
| 프론트엔드 | React 18 + Vite 5 (순수 CSS, 별도 UI 라이브러리 없음) — Cloud Run 배포 |
...
| 실시간 통신 | SSE(Server-Sent Events) — `fetch` + `ReadableStream`으로 프론트에서 직접 파싱 |
...
│   ├── api/routes/
│   │   ├── query.py             # POST /api/query — SSE 스트리밍 처리 (단일 시도 버전, 2026-07-13 실제 계약 반영)
...
│   ├── App.jsx                    # 채팅 UI 전체
│   ├── index.css
│   ├── api/queryStream.js         # SSE 스트림 fetch 파싱
│   └── components/ResultTable.jsx # 결과 표 렌더링
...
브라우저 기본 `EventSource`는 GET 요청만 지원해서 POST 바디(질문 내용)를 보낼 수 없습니다. 그래서 `src/api/queryStream.js`에서 `fetch` + `ReadableStream`으로 SSE 프레이밍(`event: ...\ndata: ...\n\n`)을 직접 파싱합니다. `App.jsx`는 이벤트 타입별로 화면을 갱신합니다.

| 이벤트 | 화면 반응 |
|---|---|
| `status` | 말풍선에 "쿼리를 생성하는 중…" 같은 상태 문구 표시 |
| `result` | 표(`ResultTable`) + 자연어 요약 + 추천 후속 질문 칩 표시 |
| `sql` | "SQL 보기" 토글 버튼 활성화 (기본은 숨김) |
| `error` | 에러 전용 스타일로 메시지 표시 |
| `done` | 스트림 종료 |

세션 식별자(`session_id`)는 브라우저 탭을 새로고침하기 전까지 하나로 유지되며, 후속 질문("그 중에 1위만 알려줘" 등) 시 백엔드가 이전 대화 맥락을 이어갈 수 있도록 매 요청에 함께 전달됩니다.

**추천 후속 질문 칩 (완료·배포·프로덕션 검증 완료, 2026-07-13)**

`result` 이벤트의 `suggestions` 배열(보통 2개)을 답변 아래 클릭 가능한 pill 버튼으로 렌더링합니다(`App.jsx`의 `suggestion-chip`). 클릭하면 입력창을 거치지 않고 그 문구가 바로 새 사용자 질문으로 전송됩니다(`handleSend`가 질문 문자열을 인자로 받도록 리팩터링됨). 스트리밍 중에는 칩이 비활성화됩니다.
...
```bash
cd Frontend
docker build -t text2sql-frontend --build-arg VITE_API_BASE_URL=http://localhost:8080 .
